...
const ProjectCard = ({ project, setShowModal, setModalData }) => {
    return (
        <div
            className={`flex w-[350px] h-[250px] relative shadow-xl border-[1px] border-black/15 cursor-pointer rounded-md`}
            onClick={() => {
                setShowModal(true), setModalData(project);
            }}
        >
            <motion.img whileHover={{ scale: 1.1 }} src={project.image.url} className="absolute  rounded-md" />
        </div>
    );
};
...
const Modal = ({ project, setShowModal }) => {
    return (
        <motion.div className=" px-[5%] lg:px-[10%] relative flex flex-col sm:flex-row-reverse gap-3 w-full h-full lg:h-[60%] lg:mt-[5%] lg:w-[80%] lg:mx-auto bg-white text-black backdrop-blur-sm">
            <IoCloseOutline
